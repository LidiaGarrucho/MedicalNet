...
from model_sigmoid import generate_model
import torch
import numpy as np
...
def train(data_loader, val_loader, model, optimizer, scheduler, total_epochs, save_interval, save_folder, sets):
    ...
    for epoch in range(total_epochs):
        ...
        if sets.enable_wandb:
            ...
            wandb.log({"epoch": epoch + 1, "loss": epoch_loss, "lr_last": scheduler.get_last_lr()[1]})
...
if __name__ == '__main__':
    ...
    sets.input_W = 150
    sets.resume_path = ''
    sets.learning_rate = 0.001 # set to 0.001 when finetune
    sets.batch_size = 8
    sets.num_workers = 4
    sets.n_epochs = 100
    ...
    # sets.model_depth = 10
    # sets.resnet_shortcut = 'B'
    sets.enable_wandb = False

    os.environ["NUMEXPR_MAX_THREADS"]=str(8)

    experiment_name = 'resnet_18_23_sigmoid_1'
